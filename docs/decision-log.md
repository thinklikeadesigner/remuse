...
| 2026-05-09 | Select MVSEP as first de-reverb and stem-separation provider. | master-orchestrator | Build Phase 2 around MVSEP `sep_type=22` for de-reverb and `sep_type=30` with `add_opt2=11` for Ensemble All-In instrument stems, with AudioShake as the production-grade stem backup and Neural Analog as the de-reverb backup. |
...
| 2026-05-09 | MVSEP instrument separation uses Ensemble All-In. | audio-provider-dev | Use `sep_type=30`, `add_opt1=0`, and `add_opt2=11` for `Ensemble All-In` with `SDR average: 13.67 (v. 2025.06.30)`. |
