...
| 2026-05-09 | MVSEP provider lane is opt-in behind `REMUSE_PROVIDER=mvsep`. | audio-provider-dev | Keep mocks as the default local path; real MVSEP mode requires `MVSEP_API_TOKEN`, persists downloaded provider artifacts, and currently requests MVSEP WAV 16-bit output until local 24-bit normalization is added. |
